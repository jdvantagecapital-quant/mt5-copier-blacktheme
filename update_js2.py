...
<script>
// DASHBOARD v3.1 - Individual Card Filters + Activity Fix
...
let activeTab = {};
let cardFilters = {};  // Individual filter per card: { master: '30days', child_0: 'today' }
let isLoading = false;
...
function getCardDateParams(cardId) {
    const filterType = cardFilters[cardId] || '30days';
...
function setCardFilter(cardId, type, btn) {
    cardFilters[cardId] = type;
    // Update buttons for this card only
    ...
    // Re-render just that card's data (filter closed trades locally)
    renderAccounts();
}

// Alias for P/L section (uses master filter)
function setPnlFilter(type, btn) {
    cardFilters['master'] = type;
    document.querySelectorAll('.pnl-filter-btn').forEach(b => b.classList.remove('active'));
    if (btn) btn.classList.add('active');
    loadData();  // Reload to get filtered closed trades
}
...
function applyPnlCustomDate() {
    ...
    if (from && to) {
        cardFilters['master'] = 'custom';
        cardFilters['master_from'] = from;
        cardFilters['master_to'] = to;
        document.querySelectorAll('.pnl-filter-btn').forEach(b => b.classList.remove('active'));
        loadData();
    }
}
...
function selectPair() {
    ...
    activeTab = {};
    cardFilters = { master: '30days' };  // Reset filters on pair change
    loadData();
}
...
async function loadData() {
    ...
    try {
        ...
        // 2. Get all data with 30-day window (we filter locally per card)
        const today = new Date(); today.setHours(0,0,0,0);
        const s30 = new Date(today); s30.setDate(s30.getDate()-30);
        const url = '/api/pairs/' + selectedPairId + '/mt5-data?date_from=' + formatDate(s30) + '&date_to=' + formatDate(today);
...
function parseActivity(activity) {
    if (typeof activity === 'string') {
        ...
        if (parts.length >= 3) {
            ...
            return { time: timeBits.length > 1 ? timeBits[1] : timePart, type: parts[1] || 'INFO', message: parts.slice(2).join(' | ') };
        }
        return { time: '', type: 'INFO', message: activity };
    }
    return { time: activity.time || '', type: activity.type || 'INFO', message: activity.message || activity.msg || String(activity) };
}
...
function renderAccounts() {
    ...
    // Get master activities - check multiple sources
    let masterActivities = [];
    if (tradeData.activities) {
        if (Array.isArray(tradeData.activities.master)) {
            masterActivities = tradeData.activities.master;
        } else if (Array.isArray(tradeData.activities)) {
            masterActivities = tradeData.activities;
        }
    }
    console.log('Master activities:', masterActivities);
    
    const filteredClosedMaster = filterByDate(tradeData.closed_master || tradeData.master?.closed_trades || [], 'master');
    const masterError = tradeData.master?.error;
    
    if (!cardFilters['master']) cardFilters['master'] = '30days';
    let html = buildCard('master', 'master', selectedPair.master_account, tradeData.balance || tradeData.master?.balance || 0, tradeData.equity || tradeData.master?.equity || 0, masterPositions, masterPnl, isRunning, masterActivities, filteredClosedMaster, masterError);
    ...
    if (children.length > 0) {
        children.forEach((child, i) => {
            ...
            let childActivities = [];
            if (tradeData.activities && tradeData.activities[cid]) {
                childActivities = Array.isArray(tradeData.activities[cid]) ? tradeData.activities[cid] : [];
            }
            console.log('Child ' + cid + ' activities:', childActivities);
            
            const filteredClosedChild = filterByDate(tradeData.closed_children?.[cid] || [], cardId);
            const childConnected = isRunning && child.enabled;
            const childError = childInfo.error;
            html += buildCard(cardId, 'child', child.account, childInfo.balance||0, childInfo.equity||0, childPositions, childPnl, childConnected, childActivities, filteredClosedChild, childError);
        });
    } else {
...
function buildCard(cardId, type, account, balance, equity, trades, pnl, connected, activities, closedTrades, error) {
    ...
    const currentFilter = cardFilters[cardId] || '30days';
    ...
    // Individual filter buttons per card
    ...
    html += '<div class="acc-tabs"><button class="tab-btn '+(currentTab==='live'?'active':'')+'" onclick="switchTab(\''+cardId+'\',\'live\')">Live ('+trades.length+')</button><button class="tab-btn '+(currentTab==='closed'?'active':'')+'" onclick="switchTab(\''+cardId+'\',\'closed\')">Closed ('+(closedTrades?.length||0)+')</button><button class="tab-btn '+(currentTab==='activity'?'active':'')+'" onclick="switchTab(\''+cardId+'\',\'activity\')">Activity ('+activityCount+')</button></div>';
    ...
    if (activities && activities.length > 0) {
        const parsed = activities.slice(0, 50).map(a => parseActivity(a));
        parsed.forEach(a => {
            const icons = { TRADE:'arrow-up', CLOSE:'arrow-down', SIGNAL:'bolt', INFO:'info-circle', ERROR:'exclamation-triangle', OPEN:'arrow-up', COPY:'copy', SYNC:'sync' };
            const icon = icons[a.type] || 'info-circle';
            html += '<div class="activity-item"><span class="activity-time">'+a.time+'</span><span class="activity-icon '+a.type.toLowerCase()+'"><i class="fas fa-'+icon+'"></i></span><span class="activity-msg">'+a.message+'</span></div>';
        });
    } else { 
        html += '<div class="empty-msg">No recent activity</div>'; 
    }
    html += '</div></div>';
...
print('Dashboard v3.1 - Individual card filters + Activity fix!')
